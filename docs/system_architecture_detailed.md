# PharmaOS Detailed System Architecture Guide
...
Direct workflow runs use structured workflow inputs built from flags or `--input-json`:

```bash
python -m pharma_os run clinical_outcome_prediction --nct-id NCT04903795
python -m pharma_os run due_diligence --nct-id NCT04903795
python -m pharma_os run protocol_design --nct-id NCT04903795
```

Control Tower orchestration uses `python -m pharma_os orchestrate --goal "<objective>"`. Natural-language goals are intentionally AI-first: `RequestUnderstandingAgent` extracts target capability, NCT ID, decision type, assumptions, refresh hints, skip hints, requested outputs, and execution scope. Explicit CLI fields override or validate the AI parse. `--input-json` is the structured non-AI bypass for a complete `OrchestrationRequest`.
...
9. Update `README.md`, `AGENTS.md`, and architecture docs in the same change.
